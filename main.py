...
class Application(tk.Tk):
    ...
    def __init__(self):
        ...
        self.lbl.grid(row=10)
        self.lblVl = tk.Label(self, text="")
        self.lblVl.grid(row=1, column=3)
        self.generuj()
        self.entry = Entry(self, width=18)
    ...
    def kontrola(self, event):
        try:
            if int(self.entry.get()) == self.vysledek:
                self.lblVl.config(text="SPRÁVNÝ VÝSLEDEK", fg="green")

            else:
                self.lblVl.config(text="ŠPATNÝ VÝSLEDEK",fg="red")
    ...
    def plus(self):
        ...
        self.cisloB = random.randint(1, 99 - self.cisloA)
